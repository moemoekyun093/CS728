...
def _tanh_sat_dist(v: torch.Tensor) -> torch.Tensor:
    # v in [-1,1]
    return 1.0 - v.abs().clamp(max=1.0)
...
def grad_time_profile(task, model, x: torch.Tensor, y_onehot: torch.Tensor, collect_extras: bool = False):
    ...
    dH = torch.autograd.grad(loss, h, create_graph=False, retain_graph=False)[0]  # (T,B,H)
    g_t = torch.linalg.vector_norm(dH, ord=2, dim=2).mean(dim=1)  # (T,)

    if hasattr(model, "act_deriv_from_h"):
        a = model.act_deriv_from_h(h)
    else:
        # GRU hidden state uses tanh candidate; this is a reasonable proxy for
        # activation sensitivity diagnostics.
        a = 1.0 - h * h
    a_t = a.mean(dim=(1, 2))
