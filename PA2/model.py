...
def _tanh_saturation_distance(h: torch.Tensor) -> torch.Tensor:
    ...
    """
    return 1.0 - h.abs().clamp(max=1.0)
...
def _sigmoid_saturation_distance(h: torch.Tensor) -> torch.Tensor:
    ...
    """
    return torch.minimum(h, 1.0 - h).clamp(min=0.0)
...
class VanillaRNN(nn.Module):
    ...
    def forward(self, u: torch.Tensor):
        ...
        for t in range(T):
            pre_h = u[t] @ self.W_uh + h_prev @ self.W_hh + self.b_hh
            h_prev = self.act(pre_h)
            hs.append(h_prev)

        h = torch.stack(hs, dim=0)
        y_all = h @ self.W_hy + self.b_hy

        if self.classif_type == "softmax":
            return y_all.reshape(T * B, self.nout), h
...
class GRUModel(nn.Module):
    ...
    def forward(self, u: torch.Tensor, return_extras: bool = False):
        ...
        for t in range(T):
            z_pre = u[t] @ self.W_uz + h_prev @ self.W_hz + self.b_z
            r_pre = u[t] @ self.W_ur + h_prev @ self.W_hr + self.b_r
            z_t = torch.sigmoid(z_pre)
            r_t = torch.sigmoid(r_pre)

            h_tilde_pre = u[t] @ self.W_uh + (r_t * h_prev) @ self.W_hh + self.b_h
            h_tilde = torch.tanh(h_tilde_pre)

            h_prev = (1.0 - z_t) * h_prev + z_t * h_tilde

            hs.append(h_prev)
            if return_extras:
                z_pres.append(z_pre)
                r_pres.append(r_pre)
                h_tilde_pres.append(h_tilde_pre)

        h = torch.stack(hs, dim=0)
        y_all = h @ self.W_hy + self.b_y
        ...
        extras = {
            "z": torch.stack(z_pres, dim=0),
            "r": torch.stack(r_pres, dim=0),
            "h_tilde": torch.stack(h_tilde_pres, dim=0),
        }
